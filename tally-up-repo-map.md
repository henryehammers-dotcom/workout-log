...
static markup with empty containers that JS fills in. ~1400 lines. Needed
for: any UI/markup change (new fields, buttons, popups, layout).
...
**`version.json`**
Single field, current build number (e.g. `"v163"`). Bumped on deploy; paired
with the service worker cache name to invalidate old cached files. Needed
for: shipping a new version (bump this + `service-worker.js` CACHE name).
...
Needed for: adding/removing a source file from the project, or changing
...
**`src/state.js`**
Central shared state. Holds: localStorage keys, `MUSCLE_GROUPS_V2` (the
group/sub-group taxonomy — chest/back/shoulders/arms/legs/core/fullbody/
cardio, each with its sub-category list), the live mutable
...
onto `window` so inline `onclick="..."` in index.html can find them, (2)
runs init on load — theme, loading schedule/library, ID migration, auto-
update checking, service worker registration. Needed for: wiring a new
function to an `onclick` handler (must be added to the `Object.assign
(window, {...})` list here), init/startup behavior changes.
...
list, logging a set against a specific date. Needed for: calendar view
changes, date navigation, PR/trend badge logic.
...
schedule/history/settings. Needed for: backup/restore/export flows.
...
units (lbs/kg) toggle, Settings sheet, first-launch Welcome flow. Needed
for: appearance, units, settings sheet, welcome flow.
...
advance + loop. Needed for: music player changes.
